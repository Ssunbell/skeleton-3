...
```
python evaluation.py --gold [gold file] --pred [predicted file] --etype [evaluation type] --db [database dir] --table [table file] --plug_value --keep_distinct --progress_bar_for_each_datapoint
...
```
python evaluate_classical.py --gold [gold file] --pred [predicted file] --out_file [output file] --num_processes [process number]
...
```
python evaluate_classical.py --pred=evaluation_examples/classical_test_gold.txt --out_file=all_eval_results.json
```

You can also choose to evaluate only on a subset of the datapoints, for example

```
python evaluate_classical.py --pred=evaluation_examples/academic_gold.txt --subset=academic --out_file=out_academic_test.json
```
